...
@implementation TWSubmitController
...
- (void)setattribute{
    ...
    backView.backgroundColor = [UIColor colorWithRed:247.0/256.0f green:247.0/256.0f blue:247.0/256.0f alpha:0.0];
    ...
    [imageview mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(backView.mas_centerX).offset(-95);
    ...
    [conuntLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(backView.mas_centerX).offset(10);
    ...
    if ([UserInfo shareUserInfoSingleton].userDataModel.mobile) {
       ...
       textlabel = [NSString stringWithFormat:@"尊敬的%@,感谢您的操作。\n您的资料已成功上传,请等待审核......\n审核通过后我们将知信通知您.",userModel.mobile];
        
    }else{
        textlabel = [NSString stringWithFormat:@"尊敬的%@,感谢您的操作。\n您的资料已成功上传,请等待审核......\n审核通过后我们将知信通知您.",_userIphone];
    }
    ...
    [attribute addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0 weight:2.0] range:NSMakeRange(45, 14)];
    ...
    [codeBut setImage:[UIImage imageNamed:@"022.jpg"] forState:UIControlStateNormal];
    codeBut.selected = YES;
    [codeBut addTarget:self action:@selector(addClickCode) forControlEvents:UIControlEventTouchUpInside];
    ...
    [DistriBut addTarget:self action:@selector(FenXiaoCode) forControlEvents:UIControlEventTouchUpInside];
...
//    if (userModel.stores_id != userModel.mobile) {
        string = [NSString stringWithFormat:@"http://www.tianview.com/api-ydz_get_erweima.html?store_id=%@&type=erweimafx",_storye_ID_1];
   // }else{
      //  string = [NSString stringWithFormat:@"http://www.tianview.com/api-ydz_get_erweima.html?store_id=%@&type=erweimadp",_useriphone];
  //  }
...
- (void)addClickCode{
    ...
    if (codeBut.selected == YES) {
        ...
        [UIView animateWithDuration:0.1 animations:^{
            ...
            self.imageView = [[UIImageView alloc]init];
           // self.imageView.image = [UIImage imageNamed:@"022.jpg"];
            ...
            self.tap1  = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(viewBackclickTWO)];
            [self.viewBack addGestureRecognizer:self.tap1];
            
            
            //            //保存图片手势
            self.tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(imageTapClickCode)];
...
#define mark二维码图片手势
- (void)imageTapClickCode{

    [self saveImageToPhotos:self.imageView.image];
    
    
    
    if (self.imageView.userInteractionEnabled == YES) {
    ...
    if ( DistriBut.selected == YES) {
        ...
        [UIView animateWithDuration:0.1 animations:^{
           ...
           // self.imageViewFenXiao.image = [UIImage imageNamed:@"011.jpg"];
            ...
            self.tap1  = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(viewBackclickTHREE)];
            [self.viewBack addGestureRecognizer:self.tap1];
            
            
            //            //保存图片手势
            self.tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(imageTapClick)];
            self.tap.numberOfTapsRequired = 1;
            self.tap.numberOfTouchesRequired = 1;
